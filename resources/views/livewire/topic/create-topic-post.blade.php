<div class="max-w-2xl mx-auto my-10 px-4 py-6 bg-white dark:bg-gray-800 rounded shadow-xl">
    ...
    {{-- tags --}}
    <div class="mb-4">
        <label for="categories" class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2">Categories (comma
            separated):</label>
        <input wire:model="categories" type="text"
            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 dark:text-gray-300 dark:bg-gray-700 leading-tight focus:outline-none focus:shadow-outline">
        @error('categories')
            <span class="text-red-500 text-xs">{{ $message }}</span>
        @enderror
