    ...
    <div class="container mx-auto px-4 md:py-12">
        ...
        <article class="mb-12 bg-gray-100 dark:bg-gray-800 p-6 rounded-lg shadow-md">
            <header class="mb-4 flex justify-between items-center">
                <span class="text-gray-600 dark:text-gray-400  ">By: <span class="dark:text-indigo-600"> {{$post->user->name}}</span> </span>
...
<div class="flex justify-end pb-12  items-baseline md:justify-center md:items-center md:p-4">
    ...
    @endif
    <span class="text-sm dark:text-gray-500 mx-4">{{ $post->likes_count }} {{ Str::plural('like', $post->likes_count) }}</span>
</div>
        <!-- Replies/Comments Section -->
        <section>
            <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-300 mb-6">Replies ({{$post->replies->count()}})</h2>

            <!-- Individual Reply -->
             @forelse ($post->replies as $reply)
<div class="mb-8 bg-gray-100 dark:bg-gray-800 p-4 rounded-lg">
                <header class="mb-4 flex justify-between">
                    <span class="text-gray-600 dark:text-gray-400">{{$reply->user?->name}}</span>
